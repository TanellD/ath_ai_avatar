...
export const AVATAR_MODELS = {
  ...
  vincent: {
    ...
    // false, и это уже не обходной путь. Шаблоны поз TalkingHead написаны под
    // T-позу (у avatar-aith руки в rest смотрят строго горизонтально), а риг
    // Vincent T-позой не является — гонять его через эти шаблоны всё равно
    // означало бы доворачивать руки на угол, посчитанный для другой геометрии.
    // Естественная поза покоя (руки опущены вдоль тела) с #37 запечена прямо
    // в rest-позу vincent.glb — раньше здесь была A-поза с разведёнными на
    // 34.5° руками, чинить которую в рантайме было нечем. Лицо, виземы,
    // моргание и взгляд по-прежнему управляются TalkingHead независимо от
    // этого флага — привязаны к другим костям, которые не трогали.
    humanoidPose: false,
    cameraView: 'head',
