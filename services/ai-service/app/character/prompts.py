...
Одного правила в шапке промпта оказалось мало. Блоки открывающих реплик
приписываются ПОСЛЕ него, и тот, что ведёт персонажа через молчание
собеседника, предписывал «новый вопрос» безусловно — модель читала его как
более позднее и потому главное указание и перехватывала ведение. Инициатива
учитывается и там, см. `_opening_block`.
"""
...
_SILENCE_CONTINUE_FOLLOW = """
Собеседник не ответил и после мягкого напоминания. Не повторяй его дословно и
не стыди за молчание. Продвинь сцену, оставаясь на своей стороне разговора:
дополни собственный предыдущий ответ, приведи пример или спроси, что ещё
рассказать. Ведение остаётся за собеседником — не начинай расспрашивать его о
работе, задачах или решениях, даже чтобы заполнить паузу. Не отвечай за
собеседника и не раскрывай критерий этапа.
"""
"""Тот же ход, но для персонажа, который инициативу НЕ держит.

Общий текст предписывал «новый вопрос» безусловно и приписывался ПОСЛЕ правила
инициативы — модель читала его как более позднее и конкретное указание,
отменяющее «не расспрашивай собеседника». Живой прогон: кандидат, у которого
holds_initiative=False доехал до сессии, всё равно спрашивал интервьюера про
его же резюме. Молчание собеседника — самый соблазнительный момент перехватить
ведение, поэтому запрет повторяется именно здесь, а не считается унаследованным
из шапки промпта.
"""
...
_OPENING_BLOCKS = {
    ...
    OpeningKind.SILENCE_CONTINUE: _SILENCE_CONTINUE_LEAD,
}
...
def _opening_block(kind: OpeningKind, holds_initiative: bool) -> str:
    """Блок открывающей реплики с поправкой на инициативу.

    От неё зависит ровно один вид: продолжение после молчания. Остальные три
    нейтральны — начало сессии прямо оговаривает случай кандидата, а переход
    этапа и мягкое напоминание одинаково уместны в обеих ролях.
    """
    if kind is OpeningKind.SILENCE_CONTINUE and not holds_initiative:
        return _SILENCE_CONTINUE_FOLLOW
    return _OPENING_BLOCKS[kind]
...
def build_character_system(
    ...
    if opening_kind is not None:
        prompt += _opening_block(opening_kind, persona.holds_initiative).format(
