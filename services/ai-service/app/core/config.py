...
class Settings(BaseSettings):
    ...
    #
    # Было 300 — нулевой запас на случай, если рассуждение думающей модели
    # (qwen3.6:35b и т.п.) хоть раз просочится мимо reasoning_effort=none и
    # _strip_inline_reasoning: живой замер на проде показал 1196 символов
    # размышления при max_tokens=300 — бюджет уходил целиком туда,
    # `finish_reason: "length"`, реплика пустая, сотрудник слышит тишину.
    # 700 — тот же запас, что подтверждён замером (реплика 194-203 симв.
    # при 600-700 бюджета), дёшево: обычная реплика укладывается в 80-170
    # символов, лишний бюджет не тратится, если рассуждения нет вовсе.
    character_max_tokens: int = 700
