...
Аватар описан одной записью в реестре
`services/scenario-service/app/avatars/registry.json`. Новый аватар — новая
запись; ни код сервисов, ни фронтенд менять не нужно:
...
```json
{
  ...
  "body": "M",
  "voice_id": "<id голоса; список показывает make voices>",
  "recovery_line": "Мур? Не расслышал. Повтори-ка."
}
```

`model_url` — путь внутри `services/frontend/public`. `body` — тип рига
TalkingHead. Сценарий ссылается на аватар полем `persona.avatar_id`.

Голос и фраза наследуются: аватар задаёт значения по умолчанию, персона может
их перекрыть своими `voice_id` и `recovery_line`. Это нужно потому, что одна
модель может достаться разным характерам — и звучать они должны по-разному.
...
упал. `make voice-recovery-setup` обходит реестр аватаров и персоны и
синтезирует по одной заготовке на каждый нужный голос. После правки реестра или
сценария команду нужно повторить.

Фраза по умолчанию не содержит прошедшего времени: род персонажа заранее
неизвестен, а «не расслышал» и «не расслышала» разошлись бы.
